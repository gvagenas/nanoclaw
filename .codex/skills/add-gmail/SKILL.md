...
**Note:** This integration applies to the **Claude provider** (MCP servers). Codex does not currently support MCP servers, so if a group uses Codex you must switch it to Claude to use Gmail MCP.
...
Append to `groups/global/CLAUDE.md` (the global memory file):
