...
**Note:** This integration applies to the **Claude provider** (MCP servers). Codex does not currently support MCP servers, so if a group uses Codex you must switch it to Claude to use Parallel AI.
...
Add `PARALLEL_API_KEY` to allowed environment variables in `src/container-runner.ts` (if not already present).

Ensure the allowlist includes `PARALLEL_API_KEY` alongside existing auth variables, for example:
```typescript
const allowedVars = [
