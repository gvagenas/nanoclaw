...
This guide covers debugging the containerized agent execution system for both Codex and Claude providers.
...
src/container-runner.ts               container/agent-runner/
    │                                      │
    │ spawns Apple Container               │ runs Claude Agent SDK or Codex CLI
    │ with volume mounts                   │ (Codex uses filesystem IPC)
    │                                      │
    ├── data/env/env ──────────────> /workspace/env-dir/env
    ├── groups/{folder} ───────────> /workspace/group
    ├── data/ipc/{folder} ────────> /workspace/ipc
    ├── data/sessions/{folder}/.claude/ ──> /home/node/.claude/ (Claude only)
    ├── data/codex/{folder}/.codex/ ─────> /home/node/.codex/ (Codex only)
    └── (main only) project root ──> /workspace/project
```

**Important:** The container runs as user `node` with `HOME=/home/node`. Claude sessions must mount to `/home/node/.claude/` and Codex config/auth to `/home/node/.codex/`.
...
| **Codex state** | `~/.codex/` | Codex auth/config/history (if enabled) |
...
```
**Fix:** Ensure `.env` file exists with either OAuth token or API key for Claude, or an OpenAI API key for Codex:
```bash
...
**Workaround:** The system extracts only authentication variables (`CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `CODEX_API_KEY`) from `.env` and mounts them for sourcing inside the container. Other env vars are not exposed.
...
echo '{}' | container run -i \
  ...
  -c 'export $(cat /workspace/env-dir/env | xargs); echo "Claude OAuth: ${#CLAUDE_CODE_OAUTH_TOKEN} chars, Claude API: ${#ANTHROPIC_API_KEY} chars, OpenAI API: ${#OPENAI_API_KEY} chars"'
```
...
```
/workspace/
├── env-dir/env           # Environment file (Claude or OpenAI auth)
...
### 5. Session Not Resuming / "Claude Code process exited with code 1" (Claude only)
...
### Test Claude Code directly (Claude only):
```bash
container run --rm --entrypoint /bin/bash \
  --mount "type=bind,source=$(pwd)/data/env,target=/workspace/env-dir,readonly" \
  nanoclaw-agent:latest -c '
...
The agent-runner uses these Claude Agent SDK options (Claude only):
