...
Ask the user:
> Do you want to use your **ChatGPT subscription** or an **OpenAI API key**?
...
> codex login
> ```
> A browser window will open for you to log in. Once authenticated, copy the auth file to:
...
**Copy existing:**
```bash
grep "^OPENAI_API_KEY=" /path/to/source/.env > .env
```
...
Tell the user to add their key from https://platform.openai.com/
...
This creates the `nanoclaw-agent:latest` image with Node.js, Chromium, Codex CLI, Claude Code CLI, and agent-browser.
